...
def load_csm_1b_local(model_path: str, device: str = "cuda", audio_num_codebooks: int = 32):
    ...
    try:
        ...
        # Download the model file if using HF model ID
        import os

        # Check if it's a local path
        local_model_file = os.path.join(model_path, "model.safetensors") if not "/" in model_path or model_path.startswith("/") else None

        if local_model_file and os.path.exists(local_model_file):
            # Check if it's a real file or just LFS pointer
            file_size = os.path.getsize(local_model_file)
            if file_size < 1000:  # LFS pointer files are tiny
                print(f"Local file appears to be LFS pointer (size: {file_size} bytes)")
                print(f"Downloading actual model from HuggingFace: sesame/csm-1b")
                model_file = hf_hub_download(repo_id="sesame/csm-1b", filename="model.safetensors")
            else:
                print(f"Loading model from local file: {local_model_file}")
                model_file = local_model_file
        else:
            # Download from HuggingFace
            repo_id = model_path if "/" in model_path and not model_path.startswith("/") else "sesame/csm-1b"
            print(f"Downloading model weights from HuggingFace: {repo_id}")
            model_file = hf_hub_download(repo_id=repo_id, filename="model.safetensors")

        state_dict = safetensors.torch.load_file(model_file, device="cpu")

        # Convert all weights to float16 to avoid dtype mismatches
        ...
        print("Model will use random initialization - results will be poor!")
    model.eval()

    # Force float16 for RTX 2060 SUPER compatibility (doesn't support bfloat16 natively)
    dtype = torch.float16

    # Disable aggressive compilation for RTX 2060 SUPER compatibility
    ...
    print("Skipping torch.compile for RTX 2060 SUPER compatibility")
    ...
    # Skip warmup for now due to CUDA errors
    print("Skipping model warmup (temporary fix for CUDA errors)")
...
def load_csm_1b(device: str = "cuda") -> Generator:
    ...
    """
    # Enable CUDA optimizations for RTX 2060 SUPER
    torch.backends.cuda.matmul.allow_tf32 = False  # RTX 2060 SUPER doesn't have tensor cores
