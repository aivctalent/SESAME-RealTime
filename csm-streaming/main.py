...
from llm_interface_api import LLMInterface
...
from transcribe_api import transcribe_audio_openai

# Check for HuggingFace authentication early
def ensure_hf_auth():
    """Ensure HuggingFace authentication is set up"""
    token = HfFolder.get_token()
    if not token:
        logger.warning("No HuggingFace token found. Attempting to login...")
        token = os.environ.get("HF_TOKEN") or os.environ.get("HUGGING_FACE_HUB_TOKEN")
        if token:
            login(token=token, add_to_git_credential=False)
            logger.info("Logged in to HuggingFace using environment variable token")
        else:
            logger.warning("No HF_TOKEN environment variable found.")
            logger.warning("To download gated models like sesame/csm-1b, please:")
            logger.warning("1. Get your token from https://huggingface.co/settings/tokens")
            logger.warning("2. Run: huggingface-cli login")
            logger.warning("   OR set HF_TOKEN environment variable")
            logger.warning("   OR run: export HF_TOKEN='your_token_here'")
    else:
        logger.info("HuggingFace authentication found")
...
# Ensure HuggingFace authentication is available
ensure_hf_auth()
model_thread = None
...
# Only load local Whisper if no API key is available
if os.environ.get("OPENAI_API_KEY") or os.environ.get("GROQ_API_KEY"):
    logger.info("API key found - will use cloud STT, skipping local Whisper model")
    whisper_model = None
    whisper_pipe = None
    processor = None
else:
    logger.info("No API key found - loading local Whisper model")
...
def transcribe_audio(audio_data, sample_rate):
    # Try API transcription first (much faster, frees up GPU)
    result = transcribe_audio_openai(audio_data, sample_rate)
    if result:
        logger.info(f"Transcribed via OpenAI API: '{result}'")
        return result
